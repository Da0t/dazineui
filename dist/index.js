...
// primitives/flow-gradient/index.tsx
var flow_gradient_exports = {};
__export(flow_gradient_exports, {
...
module.exports = __toCommonJS(flow_gradient_exports);
...
function Ribbon({ color, yOffset, phaseOffset, amplitude, frequency, waveSpeed }) {
  const matRef = (0, import_react2.useRef)(null);
  const reduced = useReducedMotion();
  const uniforms = (0, import_react2.useRef)({
    uTime: { value: 0 },
...
function RibbonScene({ colors, speed }) {
  var _a;
  const { viewport } = (0, import_fiber.useThree)();
...
var import_postprocessing = require("@react-three/postprocessing");
...
function NeonTube({
  ...
  const curve = (0, import_react3.useMemo)(
    () => new THREE2.CatmullRomCurve3([...basePoints], false, "catmullrom", 0.5),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    []
  );
  const initGeom = (0, import_react3.useMemo)(
...
function NeonScene({
  ...
  const reduced = useReducedMotion();
  const tubes = (0, import_react3.useMemo)(() => [
    {
      color,
      basePoints: [
  ...
  return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("ambientLight", { intensity: 0.05 }), tubes.map((t, i) => /* @__PURE__ */ React.createElement(NeonTube, __spreadProps(__spreadValues({ key: i }, t), { speed, reduced }))), /* @__PURE__ */ React.createElement(import_postprocessing.EffectComposer, null, /* @__PURE__ */ React.createElement(
    import_postprocessing.Bloom,
    {
      intensity: 1.4,
...
function WaveSheet({
  ...
  const matRef = (0, import_react6.useRef)(null);
  const reduced = useReducedMotion();
  ...
  const uniforms = (0, import_react6.useRef)({
    uTime: { value: 0 },
    uSpeed: { value: speed },
    uPhase: { value: phaseOffset },
  ...
  (0, import_fiber3.useFrame)((state) => {
    if (reduced || !matRef.current) return;
    matRef.current.uniforms.uTime.value = state.clock.getElapsedTime();
  });
...
function WaveScene({ colors, speed }) {
  var _a, _b;
  const { viewport } = (0, import_fiber3.useThree)();
...
function WaveScene2({ color, speed, amountX, amountY, followMouse, mouseRef }) {
  const { camera } = (0, import_fiber4.useThree)();
  const reduced = useReducedMotion();
  const geoRef = (0, import_react7.useRef)(null);
  const countRef = (0, import_react7.useRef)(0);
  const numParticles = amountX * amountY;
  const { positions, scales } = (0, import_react7.useMemo)(() => {
...
function WaveGrid({
  ...
  const mouseRef = (0, import_react7.useRef)({ x: 0, y: 0 });
  const handlePointerMove = (0, import_react7.useCallback)((e) => {
